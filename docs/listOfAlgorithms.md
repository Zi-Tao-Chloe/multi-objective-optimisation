...
Alternatively, we can use the `ExecutorProblem` class and configure algorithms by setting the properties.  See
[Running Experiments](runningExperiments.md) for more details on using the `ExecutorProblem`.

<!-- java:test/org/moeaframework/snippet/AlgorithmSnippet.java [43:48] -->

```java
NondominatedPopulation results = new ExecutorProblem()
        .withProblem("UF1")
...
CMA-ES is a sophisticated covariance matrix adaptation evolution strategy algorithm for real-valued global
...
The random search algorithm simply randomly generates new solutions uniformly throughout the search space.  It is not
